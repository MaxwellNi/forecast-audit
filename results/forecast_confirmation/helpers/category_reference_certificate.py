...
def weighted_category_bias_budget(f, g, counts, mean_v, mean_w, delta=DELTA):
    """Confidence-weighted cell envelope, including rare and unobserved cells.

    Half the failure budget covers both conditional mean vectors; the other
    half covers all category masses by one-sided exact binomial intervals.
    Their simultaneous event permits using the same validation sample.
    """
    out = category_bias_budget(f,g,counts,mean_v,mean_w,delta/2)
    n = np.asarray(counts,dtype=int)
    total = int(n.sum())
    errors = out['error_v']*out['error_w']
    upper = np.ones(len(n))
    if total:
        active = n < total
        upper[active] = beta.isf(delta/(2*len(n)), n[active]+1, total-n[active])
    # Greedy solution of max p'e subject to p>=0, p<=upper, sum(p)=1.
    remaining, bound = 1., 0.
    for c in np.argsort(-errors,kind='stable'):
        mass = min(remaining,float(upper[c]))
        bound += mass*float(errors[c])
        remaining -= mass
    if remaining > 1e-10:
        raise ArithmeticError('Category probability confidence set is infeasible')
    out.update(bias_upper=float(bound),delta=float(delta),category_mass_upper=upper,
               worst_cell_upper=float(errors.max()))
    return out
            ...
            for training in (32,512,8192):
                ...
                for validation in (512,8192):
                    ...
                    means_w = np.divide(np.bincount(vz[:validation,0],weights=aw[:validation],minlength=c),
                                        counts,out=np.zeros(c),where=counts>0)
                    budget = weighted_category_bias_budget(f,g,counts,means_v,means_w)
...
def run(output, replications, workers):
    ...
    protocol=dict(frozen_utc=datetime.now(timezone.utc).isoformat(),seed=SEED,
        ...
        bound='Half-delta cell-mean Hoeffding rectangles and half-delta one-sided Clopper-Pearson category masses; simplex-maximized error product; bounded order-three U-statistic concentration.',
        ...
        provenance='Frequency-weighted follow-up after seeing rare-category conservatism of the preserved maximum-cell study. Same seed primitives; not independent confirmation. Classical concentration is not claimed as novel.',
