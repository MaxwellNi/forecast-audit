"""Independent numerical reconstruction: no producer or gate imports.

Category probability caps use inverse beta CDFs, allowances use independent
linear programs, and generic comparison matrices reconstruct rank products.
"""
...
def budget(f,g,z,v,w,delta=1e-4):
    ...
    np.divide(np.log(8*c/delta),2*n,where=n>0,out=rad)
    np.sqrt(rad,out=rad)
